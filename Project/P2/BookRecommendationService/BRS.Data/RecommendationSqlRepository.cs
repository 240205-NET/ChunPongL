...
namespace BRS.Data
{
    public class RecommendationSqlRepository : IRecommendationSqlRepository
    {
        ...
        public async Task<List<Book>> GetRecommenedBookList(int userId)
        {
            ...
            //get _categoriesSet
            _categoriesSet =  await splitAndEliminateSpace(_rawcategoriesSet);
            //!!!!!!!!!!!!!!!!!!checked!!!!!!!!!!!!!!!
        ...
        public async Task<HashSet<int>> getCategoryIdFromCategories(int userId)
        {
            ...
            await connection.CloseAsync();
            _logger.LogInformation("Executed getCategoryIdFromCategorie, returned {0} results.", _categoryId.Count);
            return _categoryId;
        }
        ...
        public async Task<List<Book>> updateTopBooksList(HashSet<string> _categoriesSet)
        {
            ...
            _logger.LogInformation("Executed updateTopBooksList, returned {0} results.", tmpBook.Count);
            return tmpBook;
        }
        ...
        public async Task<HashSet<int>> GetCategoriesID(string s)
        {
            ...
            await connection.OpenAsync();
            //Example: {A B, A C , A}
            string cmdText = "SELECT * FROM [book].[categories] WHERE name LIKE @string";
            using SqlCommand cmd = new SqlCommand(cmdText, connection);
            ...
            await connection.CloseAsync();
            _logger.LogInformation("Executed GetCategoriesID, returned {0}.", ids.Count);

            return ids;
        }


        public async Task <HashSet<string>> splitAndEliminateSpace(HashSet<string> _rawCategoriesSet)
        {
            ...
            _logger.LogInformation("Executed splitAndEliminateSpace, returned {0} results.", tmp.Count);
            return tmp;
        }
